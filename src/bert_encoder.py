...
class BERTEncoder():
    def __init__(self,model_path,from_pt=False,do_update_weights=True):
        self.config = BertConfig.from_pretrained(os.path.join(model_path, "config.json"))
        self.tokenizer = BertTokenizer.from_pretrained("./")
        self.tf_bert_model = TFBertModel.from_pretrained(model_path, config=self.config, from_pt=from_pt)
